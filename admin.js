(function() {
    ...
    // Funciones necesarias de Firestore para el helper de eliminación
    let limit, startAfter;
    let _obsequioProductId = null;
    let _inventarioParaImportar = []; // Cache para importar inventario

    // Mapas de orden duplicados para independencia del módulo
    ...
    window.initAdmin = function(dependencies) {
        if (!dependencies.db || !dependencies.mainContent || !dependencies.showMainMenu || !dependencies.showModal) {
            console.error("Admin Init Error: Faltan dependencias críticas (db, mainContent, showMainMenu, showModal)");
            return;
        }
        _db = dependencies.db;
        ...
        // Asignar funciones importadas necesarias para deleteCollection
        limit = dependencies.limit;
        startAfter = dependencies.startAfter;
        if (!_floatingControls) {
            console.warn("Admin Init Warning: Elemento floatingControls no encontrado.");
        }
                ...
                <div class="container mx-auto max-w-md">
                    <div class="bg-white/90 backdrop-blur-sm p-8 rounded-lg shadow-xl text-center">
                        ...
                        <div class="space-y-4">
                            ...
                            <button id="deepCleanBtn" class="w-full px-6 py-3 bg-red-700 text-white font-semibold rounded-lg shadow-md hover:bg-red-800">Limpieza Profunda</button>
    ...
    async function executeDeepClean() {
        ...
        // Colecciones/Documentos Públicos a eliminar
        ...
        // Definición de Colecciones/Documentos Privados a eliminar por usuario
        ...
        // Eliminar colecciones/documentos privados para CADA usuario
    ...
    function showImportExportInventarioView() {
        ...
        _mainContent.innerHTML = `
            <div class="p-4 pt-8"> <div class="container mx-auto max-w-lg"> <div class="bg-white/90 backdrop-blur-sm p-8 rounded-lg shadow-xl">
                ...
                <p class="text-center text-gray-600 mb-6 text-sm"> Exporta a Excel o importa para actualizar cantidades existentes. </p>
                <div class="space-y-4">
    ...
    function showImportInventarioView() {
        _mainContent.innerHTML = `
            <div class="p-4 pt-8"> <div class="container mx-auto max-w-4xl"> <div class="bg-white/90 backdrop-blur-sm p-8 rounded-lg shadow-xl">
                ...
                <p class="text-center text-gray-600 mb-6 text-sm"> Selecciona archivo. Columnas: <strong>Rubro, Segmento, Marca, Presentacion, CantidadActualUnidades</strong>. Solo actualiza cantidades existentes. </p>
    ...
    function handleFileUploadInventario(event) {
        ...
        const reader = new FileReader(); reader.onload = function(e) { const data = e.target.result; let jsonData = []; try { const wb = XLSX.read(data, {type:'binary'}); const ws = wb.Sheets[wb.SheetNames[0]]; jsonData = XLSX.utils.sheet_to_json(ws, {header:1}); } catch (readError) { _showModal('Error Lectura', `Error: ${readError.message}`); return; }
            ...
            const headers = jsonData[0].map(h=>(h?h.toString().toLowerCase().trim().replace(/\s+/g,''):'')); const reqHeaders=['rubro','segmento','marca','presentacion','cantidadactualunidades']; const hMap={}; let missing=false;
            reqHeaders.forEach(rh=>{ const i=headers.indexOf(rh); if(i!==-1)hMap[rh]=i; else{_showModal('Error',`Falta columna: "${rh}"`); missing=true;}}); if(missing){renderPreviewTableInventario([]); return;}
            _inventarioParaImportar = jsonData.slice(1).map((row, rIdx) => { const cantStr = (row[hMap['cantidadactualunidades']]||'0').toString().trim(); const cant = parseInt(cantStr,10); if(isNaN(cant)||!Number.isInteger(cant)||cant<0){ console.warn(`Fila ${rIdx+2}: Cantidad inválida ('${cantStr}'). Usará 0.`); return { rubro:(row[hMap['rubro']]||'').toUpperCase(), segmento:(row[hMap['segmento']]||'').toUpperCase(), marca:(row[hMap['marca']]||'').toUpperCase(), presentacion:(row[hMap['presentacion']]||''), cantidad:0, origRow:row, isValid:false, error:'Cantidad inválida'}; } return { rubro:(row[hMap['rubro']]||'').toUpperCase(), segmento:(row[hMap['segmento']]||'').toUpperCase(), marca:(row[hMap['marca']]||'').toUpperCase(), presentacion:(row[hMap['presentacion']]||''), cantidad:cant, origRow:row, isValid:true}; }).filter(i=>i.rubro&&i.segmento&&i.marca&&i.presentacion); renderPreviewTableInventario(_inventarioParaImportar);
    ...
    function renderPreviewTableInventario(items) {
        ...
        const vCount=items.filter(i=>i.isValid).length, invCount=items.length-vCount;
        let tHTML=`<div class="p-4"><h3 class="font-bold text-lg mb-2">Vista Previa (${vCount} válidos, ${invCount} inválidos)</h3> ${invCount>0?'<p class="text-sm text-red-600 mb-2">Filas rojas tienen cantidad inválida (usará 0).</p>':''} <table class="min-w-full bg-white text-xs"><thead class="bg-gray-200 sticky top-0"><tr><th>Rubro</th><th>Segmento</th><th>Marca</th><th>Presentación</th><th class="text-right">Nueva Cant. (Und)</th></tr></thead><tbody>`;
        items.forEach(i=>{const rCls=i.isValid?'':'bg-red-100 text-red-700'; tHTML+=`<tr class="border-b ${rCls}"><td class="py-1 px-2">${i.rubro}</td><td class="py-1 px-2">${i.segmento}</td><td class="py-1 px-2">${i.marca}</td><td class="py-1 px-2">${i.presentacion}</td><td class="py-1 px-2 text-right">${i.cantidad}${i.isValid?'':' (Inválido)'}</td></tr>`;}); tHTML+='</tbody></table></div>'; cont.innerHTML=tHTML;
    ...
    async function handleConfirmInventarioImport() {
        const itemsProc = _inventarioParaImportar.filter(i=>i.isValid); if(itemsProc.length===0){_showModal('Aviso','No hay productos válidos.'); return;} _showModal('Progreso',`Buscando ${itemsProc.length} productos...`);
        try { const invRef = _collection(_db, `artifacts/${_appId}/users/${_userId}/inventario`); const snap = await _getDocs(invRef); const curInvMap = new Map(); snap.docs.forEach(d=>{const data=d.data(); const key=`${data.rubro||''}|${data.segmento||''}|${data.marca||''}|${data.presentacion||''}`.toUpperCase(); curInvMap.set(key,{id:d.id, curQty:data.cantidadUnidades||0});});
            _showModal('Progreso','Preparando actualizaciones...'); const batch=_writeBatch(_db); let updates=0, notFound=0; const notFoundPs=[];
            itemsProc.forEach(i=>{ const key=`${i.rubro||''}|${i.segmento||''}|${i.marca||''}|${i.presentacion||''}`.toUpperCase(); const existP=curInvMap.get(key); if(existP){ if(existP.curQty!==i.cantidad){ const dRef=_doc(invRef,existP.id); batch.update(dRef,{cantidadUnidades:i.cantidad}); updates++;}} else {notFound++; notFoundPs.push(`${i.marca} ${i.presentacion}`);}});
            if(updates===0){_showModal('Aviso',`Sin coincidencias o sin cambios.${notFound>0?` (${notFound} no encontrados)`:''}`); showImportExportInventarioView(); return;}
            _showModal('Confirmar Importación', `Actualizar ${updates} producto(s)? ${notFound>0?`${notFound} no encontrados.` : ''} ${notFound>0?`<br><small>No encontrados: ${notFoundPs.slice(0,5).join(', ')}${notFound>5?'...':''}</small>`:''}`, async ()=>{ _showModal('Progreso',`Aplicando ${updates}...`); try { await batch.commit(); _showModal('Éxito',`Actualizados ${updates} producto(s).`, showImportExportInventarioView); } catch (commitErr) { console.error("Error commit:", commitErr); _showModal('Error',`Error al guardar: ${commitErr.message}`); } }, 'Sí, Actualizar');
        } catch (error) { console.error("Error import process:", error); _showModal('Error',`Error: ${error.message}`); }
    ...
    async function showObsequioConfigView() {
        ...
        _mainContent.innerHTML = `
            <div class="p-4 pt-8"> <div class="container mx-auto max-w-lg"> <div class="bg-white/90 backdrop-blur-sm p-8 rounded-lg shadow-xl">
                ...
                <p class="text-gray-600 mb-4 text-center text-sm">Selecciona producto obsequio (debe manejar vacíos y venderse por caja). Configuración pública.</p>
